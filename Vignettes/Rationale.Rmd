...
The way that this function works is that instead of altering the base mechanics of \adonis2, it uses loops to run through x amount of iterations of the model (x = n permutations, set according to the model chosen). A permutation object is created that stores x amount of versions of your permuted data. A results object is created to store the sums of squares for each of the x amount of permutations for every term in the model. 

Then, a loop is used to run a single iteration of the PERMANOVA model with zero permutations for every single line in the permutation object. The sums of squares from each run are saved in the results object. The actual data are preserved on the first line and the remaining lines are shuffled versions of that data (within the constraints of how you outline your permutations). From there, columns are created to calculate the F values for each permutation and the original data. And using that F distribution, the adjusted P value is calculated using the following equation: 
